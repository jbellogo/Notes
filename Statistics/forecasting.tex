...
Maybe I should do this in RMarkdown instead...
...
The time series $\{X_t\}$ is said to be \textit{weakly stationary} if:
\begin{itemize}
\item finite second absolute moment forall t
\item constant mean FORALL times
\item scale-invariant pair-wise covariance for all pairs. ie acvf satisfies $\gamma(r,s) = \gamma(r+h, s+h)$. Visually this means the period is constant. ie it doesn't contract and strectch at parts. Only depends on the distance between two variables and not on $t$.
\end{itemize}
